...
@MicronautTest
class CommandTest extends ThreshrTest {
    ...
    void "query help text"() {
        ...
        String[] outputStreams = executeCommand(Command, args)

        then:
        for (String line : helpText)
            outputStreams[0].contains(line)
    }

    def "varying tcin arg count with no error"() {
        when:
        String[] args = new String[]{"-t", tcinArgs}
        String[] streams = executeCommand(Command, args)
     ...
     */
    private static String[] executeCommand(Class commandClass, String[] args) {
